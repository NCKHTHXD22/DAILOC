...
(async () => {
  // Ưu tiên Redis — tokens trong Redis luôn mới hơn (do OAuth hoặc auto-refresh cập nhật)
  // Chỉ dùng env var khi Redis trống (lần đầu tiên chưa có token)
  ...
  if (redisAccess) {
    ...
    console.log('[ZaloToken] Dùng token từ Redis (mới nhất)');
  } else {
    // Redis trống → dùng env var và ghi vào Redis
    ...
    if (envAccess) {
      ...
      console.log('[ZaloToken] Redis trống — dùng env var và đồng bộ vào Redis');
    }
  }
