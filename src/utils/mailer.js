...
// Gửi email; im lặng bỏ qua nếu EMAIL_ADMIN chưa được cấu hình (chưa có Gmail chính thức)
async function sendMail({ to, subject, html }) {
  ...
  try {
    await t.sendMail({ from: `"UBND Xã Đại Lộc" <${CONFIG.EMAIL_ADMIN}>`, to, subject, html });
  } catch (err) {
...
function buildFeedbackEmailHtml({ heading, feedback, shortCode }) {
  ...
  return `
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;color:#1A2340;">
      ...
      <p style="margin-top:18px;font-size:12px;color:#94A3B8;">Email tự động từ hệ thống Góp ý – Phản ánh UBND Xã Đại Lộc.</p>
    </div>`;
}
