...
async function getFollowerProfile(userId) {
  // Template literal giữ userId là số nguyên, tránh lỗi -201 "user_id is not valid"
...
async function syncFollowers() {
  ...
  const redisSaved = await redisCmd('SET', 'dailoc_oa_followers', JSON.stringify(profiles));
  if (redisSaved !== null) {
    await redisCmd('SET', 'dailoc_oa_followers_synced_at', _syncedAt);
  } else {
    saveToFile(profiles);
  }
...
async function getStoredFollowers() {
  ...
  // Thử Redis
  const raw = await redisCmd('GET', 'dailoc_oa_followers');
  if (raw) {
    try {
...
async function getSyncedAt() {
  ...
  const fromRedis = await redisCmd('GET', 'dailoc_oa_followers_synced_at');
